...
treeNode* insertNode(treeNode *p, int x) {
	...
	return p;
	*/
	if (p == NULL) // 트리의 맨 하단에 도달
		return makeNode(x);	// 리턴 후 newNode를 부모와 연결
	else if (x < p->size) p->left = insertNode(p->left, x);
	else if (x > p->size) p->right = insertNode(p->right, x);
	else printf("\n 이미 같은 키가 있습니다! \n");
...
int main() {
	...
	/* 함수 미리 만들기
	treeNode* F7 = makeRootNode(7, NULL, NULL);
	treeNode* F6 = makeRootNode(5, NULL, NULL);
	treeNode* F5 = makeRootNode(3, NULL, NULL);
	treeNode* F4 = makeRootNode(1, NULL, NULL);
	treeNode* F3 = makeRootNode(6, F6, F7);
	treeNode* F2 = makeRootNode(2, F4, F5);
	root = makeRootNode(4, F2, F3);
	*/
